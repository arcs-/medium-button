...
MediumButton.prototype.checkState=function(a){a=getCurrentSelection();""!=this.options.start&&-1<a.indexOf(this.options.start)&&-1<a.indexOf(this.options.end)&&this.button.classList.add("medium-editor-button-active")};
...
ion(), e.rangeCount) { for (var a = document.createElement("div"), b = 0, d = e.rangeCount; b < d; ++b) a.appendChild(e.getRangeAt(b).cloneContents());
			a = a.innerHTML } } else "undefined" != typeof document.selection && "Text" == document.selection.type && (a = document.selection.createRange().htmlText); return a };
